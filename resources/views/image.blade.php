...
<body>
		...
		<div class="container-fluid">
			...
			<div class="search_types">
				...
				<ul class="search_types_selector nav nav-tabs" id="myTab" role="tablist">
					...
					<li class="single_search_item nav-item">
						<a href="{{ route('image', $keyword_string) }}" class="nav-link active"aria-selected="false">
	...
	<div class="search_result_area">
		<div class="container-fluid">
			<div class="row">
				<div class="col-12">
					<p class="search_item_count">Image {{ $results['searchInformation']['formattedTotalResults'] }} results({{ $results['searchInformation']['formattedSearchTime'] }} seconds)</p>
				</div>
			</div>
			<div class="row tab-content" id="myTabContent">
					...
					<!-- all home -->
					<div class="tab-pane mt-2 image_search fade show active" id="image">
						<div class="row">
							<div class="col-12">
								<div class="single_image_grid">
		
									<?php

										$imageResults = array_filter($results['items'], function($result) {
										    return isset($result['pagemap']['cse_image']) && isset($result['pagemap']['cse_image'][0]['src']) && !empty($result['pagemap']['cse_image'][0]['src']);
										});
									...
									@foreach ($imageResults as $result)
										...
										@if (!empty($result['pagemap']['cse_image'][0]['src']))
											<div class="col-md-3">
												<a href="{{ $result['link'] }}" class="single_item_box" target="_blank">
													<img src="{{ $result['pagemap']['cse_image'][0]['src'] }}" style="width: 100%;height: 300px;" class="img-fluid" alt="">
													<br>
													<!-- <span class="image_title">{{ $result['title'] }}</span> -->
												</a>
											</div>
										@endif
									@endforeach
